...
but still needs a URL to hand to ``POST /payments/{id}/receipt``. Every other
folder (arena/court images) stays owner/admin-only.
"""
...
_PLAYER_ALLOWED_FOLDERS = frozenset({"receipts"})
...
async def upload_image(
    ...
    if user.role not in (UserRole.owner, UserRole.admin) and folder not in _PLAYER_ALLOWED_FOLDERS:
        raise ForbiddenError("Players may only upload to the receipts folder.")
